  ...
  stages {
          ...
          sh """
            ...
            for i in {1..30}; do
              if curl -s http://localhost:4200 > /dev/null; then
                echo "✅ Angular frontend is up"
                break
              fi
              echo "⏳ Waiting for Angular frontend... ($i)"
              sleep 1
            done
          ...
          sh """
            ...
            for i in {1..30}; do
              if curl -s http://localhost:8086 > /dev/null; then
                echo "✅ Vue frontend is up"
                break
              fi
              echo "⏳ Waiting for Vue frontend... ($i)"
              sleep 1
            done
    ...
    stage('Deploy to Production') {
      steps {
        ...
        // Ajoute ici ton déploiement si prêt
      }
    }
